...
import { formatPrice, packages, pageMetadata, siteConfig } from "../lib/site";
...
export default function PackagesPage() {
  ...
  return (
    <main id="main-content">
      <PageHero eyebrow="Duidelijke pakketkeuze" title="Kies een lespakket dat" accent="bij jouw startpunt past." intro="Drie vaste routes met een heldere inhoud. Gebruik de configurator als je nog twijfelt; tijdens de intake controleren we samen of jouw keuze passend is."><Breadcrumbs items={[{ label: "Lespakketten" }]} /></PageHero>
      <section className="section"><div className="site-shell"><PackageCards /><AssuranceStrip /><p className="fineprint">Losse rijles: €60 per lesuur. *Herexamenregeling volgens pakketvoorwaarden. Alle tarieven moeten voor publicatie door Van Dijk worden bevestigd.</p></div></section>
      ...
      <section className="section"><div className="site-shell card-triptych"><article><Clock width="24" /><h3>Lesuren en afspraken</h3><p>Een pakket wordt uitgedrukt in lesuren van 60 minuten. Kies je afspraken van 90 minuten, dan bestaat een pakket van 30 lesuren bijvoorbeeld uit ongeveer twintig afspraken.</p></article><article><Shield width="24" /><h3>Voorwaarden vooraf duidelijk</h3><p>Voor livegang worden geldigheidsduur, annulering, betaling, examenkosten en de exacte herexamenregeling zichtbaar aan ieder pakket gekoppeld.</p></article><article><Sparkles width="24" /><h3>Later uitbreidbaar</h3><p>Blijkt tijdens de opleiding dat extra lesuren verstandig zijn? Dan bespreken we dit tijdig en blijft je lesplan transparant.</p></article></div></section>
      ...
      <JsonLd data={packages.map((item) => ({ "@context": "https://schema.org", "@type": "Product", name: item.name, description: item.description, brand: { "@type": "Brand", name: siteConfig.name }, offers: { "@type": "Offer", priceCurrency: "EUR", price: item.price, availability: "https://schema.org/InStock", url: `${siteConfig.url}/lespakketten#${item.id}` } }))} />
    </main>
  );
}
