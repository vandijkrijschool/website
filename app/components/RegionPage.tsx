...
export default function RegionPage({ region }: { region: Region }) {
  ...
  return (
    <main id="main-content">
      ...
      <section className="section"><div className="site-shell local-intro"><div><SectionHeading eyebrow={`Rijles in ${region.displayName}`} title="Afwisseling die past bij jouw ontwikkeling." /><p>{region.context}</p><ul className="checklist">{region.situations.map((item) => <li key={item}><Check width="17" /> {item}</li>)}</ul><p className="verification-flag">Lessen zijn beschikbaar overdag, ’s avonds en op zaterdag. De actuele beschikbaarheid stemmen we tijdens de intake met je af.</p></div><aside className="notice-card"><MapPin width="25" /><h3>Ophaalservice in {region.displayName}</h3><p>We halen je op bij huis, school, werk of een afgesproken station. Aan het einde van de les zetten we je weer af op de afgesproken plek.</p></aside></div></section>
      ...
      <SplitCta title={`Start jouw rijopleiding in ${region.displayName}.`} text="Geef je voorkeuren door voor een gratis proefles van 50 minuten." />
